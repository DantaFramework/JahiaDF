...
package danta.jahia.templating.tagsupport;
...
/**
 * This is the implementation of the JSP tag: <template:area /> adapted to Danta.
 *
 * @author      jarriola
 * @version     1.0.0
 * @since       2017-08-11
...
public class AreaTag extends ModuleTag implements ParamParent {
    ...
    protected void findNode(RenderContext renderContext, Resource currentResource) throws IOException {
        ...
        //JSP VERSION: pageContext.setAttribute("org.jahia.emptyArea",Boolean.TRUE, PageContext.PAGE_SCOPE);
        this.helper.getRequest().setAttribute("org.jahia.emptyArea",Boolean.TRUE);
        try {
            ...
            if ("absoluteArea".equals(moduleType)) {
                ...
                try {
                    ...
                    //JSP VERSION: pageContext.setAttribute("org.jahia.emptyArea",Boolean.FALSE, PageContext.PAGE_SCOPE);
                    this.helper.getRequest().setAttribute("org.jahia.emptyArea",Boolean.FALSE);
                } catch (RepositoryException e) {
                    if (node != null) {
...
//                                    conflictsWith = this.node.getPath();
    ...
    @Override public int doEndTag() throws JspException {
        //JSP VERSION: Object o = pageContext.getRequest().getAttribute("inArea");
        Object o = this.helper.getRequest().getAttribute("inArea");
        try {
            return super.doEndTag();
        } finally {
            //pageContext.getRequest().setAttribute("previousTemplate", templateNode);
            this.helper.getRequest().setAttribute("previousTemplate", templateNode);
            if(logger.isDebugEnabled()) {
            ...
            //JSP VERSION: pageContext.getRequest().setAttribute("inArea", o);
            //JSP VERSION: pageContext.getRequest().removeAttribute(AreaResourceCacheKeyPartGenerator.AREA_PATH);
            this.helper.getRequest().setAttribute("inArea", o);
            this.helper.getRequest().removeAttribute(AreaResourceCacheKeyPartGenerator.AREA_PATH);
        }
    }
